...
export default function MobilePreview({ data, activeTheme }: MobilePreviewProps) {
  const [viewMode, setViewMode] = useState<"card" | "kakao">("card");

  // Format worship order sequence cleanly for text copy
  const praiseList = data.praiseSongs;

  return (
    <div className="flex flex-col items-center gap-4 py-2" id="mobile-preview-container">
      ...
      <div className="relative w-full max-w-[350px] aspect-[9/19.5] bg-slate-950 rounded-[40px] p-2.5 shadow-2xl border-4 border-slate-800 ring-8 ring-slate-900 ring-offset-2 ring-offset-slate-100 flex flex-col overflow-hidden">
        ...
        <div className="flex justify-between items-center px-6 pt-3.5 pb-1 text-black/85 z-40 text-[10px] font-mono shrink-0 font-medium">
          <span className={viewMode === "card" && activeTheme.id === "theme-deep-space" ? "text-slate-400" : "text-slate-600"}>
            20:00 Sat
          </span>
          <div className="flex items-center gap-1">
            <span className={`w-3 h-1.5 rounded-xs border ${viewMode === "card" && activeTheme.id === "theme-deep-space" ? "border-slate-500 bg-emerald-500" : "border-slate-800 bg-emerald-600"}`}></span>
            <span className={viewMode === "card" && activeTheme.id === "theme-deep-space" ? "text-slate-400" : "text-slate-600"}>
              LTE
            </span>
          </div>
        ...
        <div className="flex-1 rounded-[32px] overflow-hidden relative flex flex-col">
          ...
          {viewMode === "card" && (
            <div 
              ...
              className={`flex-1 overflow-y-auto ${activeTheme.bgColor} ${activeTheme.textColor} p-5 pb-12 flex flex-col gap-6 relative select-none scrollbar-thin`}
              id="card-mode-view"
            >
              {/* Star decoration for Deep dark theme */}
              {activeTheme.id === "theme-deep-space" && (
            ...
            <div className="flex-1 overflow-y-auto bg-[#BACEE0] p-4 flex flex-col font-sans select-text scrollbar-thin" id="kakao-mode-view">
              ...
              <div className="flex gap-2.5 mt-2">
                ...
                <div className="flex flex-col gap-1 items-start min-w-0 flex-1">
                  ...
                  {/* Chat Bubble card */}
                  <div className="bg-white text-slate-900 rounded-tr-xl rounded-b-xl px-3.5 py-3 shadow-xs border border-slate-200/50 max-w-[240px] text-[10.5px] leading-relaxed whitespace-pre-wrap select-all selection:bg-teal-100 font-mono">
